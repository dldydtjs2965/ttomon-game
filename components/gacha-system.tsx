...
export function GachaSystem() {
  ...
  const performGachaPull = useGameStore((state) => state.performGachaPull)
  ...
  const handleGachaPull = async () => {
    ...
    setTimeout(() => {
      try {
        const monster = performGachaPull()
      ...
      <Card className="w-full max-w-md mx-auto">
        ...
        <CardContent className="space-y-4">
          <div className="text-center space-y-2">
            <p className="text-sm text-muted-foreground">새로운 몬스터를 뽑아보세요!</p>
          ...
          <Button onClick={handleGachaPull} disabled={isAnimating} className="w-full" size="lg">
            {isAnimating ? (
